...
export interface CompiledManuscript {
    ...
    metadataPath: string
    /**
     * Absolute path of the Lua filter the Typst engine runs to keep citations
     * out of Typst's native `#cite()`/`#bibliography()` machinery. Always
     * written. See {@link CITEPROC_TYPST_FILTER}.
     */
...
export class ManuscriptCompiler {
    ...
    async compile(book: ParsedBook, tempDir: string): Promise<CompiledManuscript> {
        ...
        // Always written: the Typst engine runs this filter even without a
        // bibliography, to neutralise stray `@token` citations that Pandoc
        // would otherwise emit as native `#cite()` (which hard-errors with no
        // `#bibliography()`). See issue #2.
...
/**
 * Pandoc Lua filter that keeps citations out of Typst's native citation
 * machinery. Pandoc's Typst writer renders every citation as a native `@key` /
 * `#cite(...)` reference and emits a native `#bibliography("<path>")` directive.
 * Two things break under Typst because of this (both surface as Pandoc's
 * generic "Error 43"):
 *
 *  1. **No bibliography → hard failure.** Any `@token` in the manuscript that
 *     Pandoc reads as a citation (intentional or a stray handle/word) becomes a
 *     native `#cite()`, and Typst aborts with "the document does not contain a
 *     bibliography" when no `#bibliography()` is present. EPUB tolerates the
 *     same input, which is why "EPUB works, PDF fails". This is the actual
 *     issue #2.
 *  2. **With a bibliography → format/duplication.** Typst's built-in reader
 *     only accepts BibLaTeX (`.bib`) / Hayagriva (`.yml`), so a CSL-JSON/YAML
 *     bibliography (which citeproc reads happily) makes the native directive
 *     abort with "unknown bibliography format"; and even for a `.bib` the
 *     reference list renders twice.
 *
 * The filter unwraps every `Cite` element to its inline content — after
 * citeproc (when a bibliography is present) that is the rendered citation text;
 * with no citeproc it is the literal `@key` text — so the writer never emits a
 * native cite. It also drops the `bibliography`/`csl` metadata so no native
 * `#bibliography()` is emitted. The runner runs it for every Typst export
 * (after `--citeproc` when citeproc is active), so Typst never touches the
 * bibliography file and citations match the EPUB output. See issue #2.
 */
