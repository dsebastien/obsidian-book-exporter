...
- **Section numbering.** New `numberSections` setting (default off) and per-book `book_export.number_sections`. Forwards `--number-sections` to pandoc — chapters and sections get hierarchical numbers (`1`, `1.1`, `1.1.1`, …) in the output and the table of contents.
- **Citations** via `pandoc-citeproc`. Add `bibliography:` (and optionally `csl:`) to the manifest's frontmatter — accepts a vault-relative path, an absolute path, or an `[[wikilink]]`. The exporter resolves the path, writes it into the metadata YAML, and passes `--citeproc` to pandoc so citations like `[@smith2020]` in your notes render with a proper bibliography section in the export.
- **Duplicate-note detection.** The validator now flags notes referenced more than once in the same manifest. Surfaces as a warning with the section breadcrumbs where the duplicates live so you can spot accidental cut-and-paste in the manifest before the export inlines the content twice.
