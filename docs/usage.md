...
| `cover`          | optional    | Vault-relative or absolute image path, an `[[wikilink]]`, or an `http(s)` URL (downloaded automatically). Used as the EPUB cover **and** as a full-bleed first page in PDF exports (Typst, xelatex, tectonic, weasyprint). The frontmatter key name is configurable in **Settings → Note processing → Cover frontmatter property**. |
...
Beyond the command palette, you can export without leaving the note: click the **book ribbon icon** in the left sidebar, or right-click a Markdown note (in the file explorer or the editor) and choose **Export book (EPUB / PDF)**. Both export the active/selected note with its default formats; if the note isn't a valid manifest you get a clear notice instead of a broken export.
...
5. Inserts page breaks when **page break per chapter** is enabled:
    ...
    - When the manifest uses two heading levels (e.g. H2 = parts, H3 = chapters), each new part starts on a fresh **right-hand (recto) page** — leaving the verso blank when needed. Implemented via format-conditional raw blocks: Typst `pagebreak(to: "odd")`, LaTeX `\cleardoublepage`, EPUB / weasyprint CSS `page-break-before: always`.
6. Applies **front matter** numbering when `book_export.front_matter_sections` is set: the listed top-level sections are numbered with lowercase roman numerals, and the first body-matter section resets to arabic `1`. Works across Typst, LaTeX, and weasyprint (see [Configuration](configuration.md#per-book-overrides-book_export)).
7. Applies **page setup** (page size, margin, line spacing, base font size) from settings or per-book overrides, translated to the right mechanism per engine.
8. Injects a small Typst preamble that styles block quotes (left rule, italicized body) so quotes render cleanly in PDF without LaTeX. Pandoc's `+smart` reader extension also turns straight quotes into curly quotes throughout.
9. Hands the result to Pandoc with a generated YAML metadata file (avoids escaping issues with non-ASCII titles).
