...
| PDF engine                 | enum       | `typst`                                               | `typst` (recommended) / `xelatex` / `tectonic`.                                                                                                                                                                                                                                                                                      |
...
The default engine is **Typst** for one reason: it gives professional book typography (proper page breaks, no split images, hyphenation, widow/orphan control) without the install pain of LaTeX. xelatex / tectonic produce comparable output if you already have them. (HTML-based engines — weasyprint, wkhtmltopdf — were removed in #36: the front/body-matter transitions and full-bleed cover are Typst/LaTeX-only, so they produced silently wrong output.)
