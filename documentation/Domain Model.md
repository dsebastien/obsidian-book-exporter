...
- **BookMetadata** — title, authors, language, optional ISBN/publisher/cover/description/bibliography/csl/etc. Title resolution: frontmatter `title` → body `# H1` (with trailing ` (Book)` stripped) → manifest basename (with trailing ` (Book)` stripped). Authors resolution: frontmatter `authors:` → plugin `defaultAuthors` setting → `["Anonymous"]`. Cover resolution: the frontmatter key is configurable (plugin setting `coverProperty`, default `cover`); the value can be a vault-relative path, an `[[wikilink]]`, an absolute filesystem path, or an `http(s)` URL (downloaded by the exporter before pandoc runs and rewritten to a local temp path). `coverPath` always points to a local file by the time pandoc is invoked. Citation resources (`bibliography:`, `csl:`) follow the same path resolution as the cover (vault-relative, absolute, `[[wikilink]]`); URLs are NOT supported here. When `bibliographyPath` is set, the exporter passes `--citeproc` to pandoc.
- **BookExportOverrides** — per-book overrides (`output_dir`, `pdf_engine`, `toc_depth`, `include_toc`, `page_break_per_chapter`, `formats`, `pandoc_extra_args`, `sections_to_skip`, `inlined_note_separator`, `front_matter_sections`).
...
When `book_export.front_matter_sections` lists one or more top-level section titles, the compiler treats those as **front matter**: numbered with lowercase roman numerals (`i`, `ii`, …). The first non-front top-level section begins **body matter**, which resets numbering to arabic (`1`, `2`, …). As with the part break, this is emitted as format-conditional raw blocks so each pandoc target gets the right mechanism:

- `{=typst}` → `#set page(numbering: "i")` for front matter; `#pagebreak(to: "odd")` + `#counter(page).update(1)` + `#set page(numbering: "1")` at the body transition.
- `{=latex}` → `\frontmatter` / `\mainmatter` (book class).
- `{=html}` (weasyprint) → the body content is wrapped in a `.body-matter` div (opened at the transition, closed at the end of the manuscript). The CSS `page` property does **not** propagate to sibling boxes, so the wrapper is required. The generated stylesheet (see Outputs) numbers front matter roman via the default `@page`, switches the wrapper to a named `@page body`, and resets the counter on `@page :nth(1 of body)` — weasyprint only honours a page-counter reset declared in an `@page` rule, not on document elements.

HTML/EPUB readers ignore print page numbering, so the `{=html}` wrapper is inert outside the weasyprint PDF path.
...
- `pdf` — Pandoc with `--pdf-engine=<engine>`. Engine is configurable globally + per book; default is Typst. Typst/xelatex/tectonic get a full-bleed cover via `--include-in-header`; **weasyprint** additionally receives a generated stylesheet (`--css`) carrying page setup, front/body-matter numbering, and the cover, plus the cover as an `--include-before-body` HTML fragment. The engine binary (LaTeX, weasyprint) is the user's to install; the path can be pinned with the `pdfEnginePath` setting (forwarded as `--pdf-engine=<path>` when its basename matches the selected engine).
