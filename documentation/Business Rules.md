...
5. **Pandoc is the single external dependency.** The plugin MUST NOT add other tool dependencies (Calibre, KindleGen, custom binaries) without explicit approval. PDF engine selection (Typst / xelatex / tectonic / weasyprint) is the user's choice; the plugin forwards the engine to Pandoc and, for the HTML engine (weasyprint), also generates the stylesheet and cover fragment that drive page setup, numbering, and the cover. The engine binary itself (a LaTeX distribution, weasyprint) is the user's to install — `pip install weasyprint`, etc.
